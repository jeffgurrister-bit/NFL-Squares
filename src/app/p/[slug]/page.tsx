...
export default async function PoolHome({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Find latest week with a determinable winner
  ...
  for (const pw of [...pool.poolWeeks].sort((a, b) => b.weekNumber - a.weekNumber)) {
    ...
    const rd = pw.rowDigits;
    const cd = pw.colDigits;
    const r = rd.indexOf(String(t.winnersDigit));
    const c = cd.indexOf(String(t.losersDigit));
    if (r === -1 || c === -1) continue;
    ...
    <>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Stat label="Current Week" value={String(pool.activeWeekNumber)} />
